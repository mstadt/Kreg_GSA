...
ms1 = '*';
ms2 = '^';
%ytickvals = [0.0, 0.25, 0.5, 0.75, 1.0, 1.25, 1.5];
yminmax = [0, 4.5];
...
for ii = 1:npars
    for jj = 1:ntimes
        plas_MIvals_all(ii, jj) = MorrisIndex(plas_mustarvals_all(ii, jj), plas_sigvals_all(ii,jj));
        musc_MIvals_all(ii, jj) = MorrisIndex(musc_mustarvals_all(ii,jj), musc_sigvals_all(ii,jj));
    end
end
...
clf;
subplot(nr, nc, 1)
plot(plas_maxMI(pIDs), 'markersize', marksize, 'marker', ms1,...
            'color', cmax, 'linestyle', 'none')
hold on
plot(plas_meanMI(pIDs), 'markersize', marksize, 'marker', ms2,...
            'color', cmean, 'linestyle', 'none')
errorbar(plas_meanMI(pIDs), plas_sdMI(pIDs),...
            'color', cmean, 'linestyle', 'none')
plot(plas_minMI(pIDs), 'markersize', marksize, 'marker', ms1,...
            'color', cmax, 'linestyle', 'none')
xticks(1:23)
xticklabels(pars_plas_sort)
title("Morris Indices for plasma concentration")
ylabel("Morris Index")
legend('maximum/minimum', 'mean/std')
set(gca, 'fontsize', 18)
ylim(yminmax)
%yticks(ytickvals)
grid on
...
subplot(nr, nc, 2)
plot(mMI, 'markersize', marksize, 'marker', ms1,...
            'color', cmax, 'linestyle', 'none')
hold on
plot(musc_meanMI(mIDs), 'markersize', marksize, 'marker', ms2,...
        'color', cmean, 'linestyle', 'none')
errorbar(musc_meanMI(mIDs), musc_sdMI(mIDs),...
        'color', cmean, 'linestyle', 'none')
plot(musc_minMI(mIDs), 'markersize', marksize, 'marker', ms1,...
        'color', cmax, 'linestyle', 'none')
xticks(1:23)
xticklabels(pars_musc_sort)
title("Morris Indices for Muscle concentration")
ylabel("Morris Index")
set(gca, 'fontsize', 18)
ylim(yminmax)
%yticks(ytickvals)
...
%%
% same but with muscle concentrations sorted same as plasma
figure(38)
marksize = 15;
marksize2 = 8;
cmax = cmap(3,:);
...
xticklabels(pars_plas_sort)
xlim([1,23])
title("Morris Indices for plasma [K^+] (Meal + KCl)")
ylabel("Morris Index")
...
xlim([1,23])
xticklabels(pars_plas_sort)
title("Morris Indices for muscle [K^+] (Meal + KCl)")
ylabel("Morris Index")
set(gca, 'fontsize', 18)
ylim(yminmax)
%yticks(ytickvals)
...
xticklabels(pars_plas_sort)
xlim([1,23])
title("Morris Indices for plasma [K^+]")
ylabel("Morris Index")
...
xlim([1,23])
xticklabels(pars_plas_sort)
title("Morris Indices for intracellular [K^+]")
ylabel("Morris Index")
set(gca, 'fontsize', 18)
ylim(yminmax)
%yticks(ytickvals)
...
AddLetters2Plots(figure(40), {'(a)', '(b)'},...
                'HShift', -0.05, 'VShift', -0.06, ...
                'fontsize', 16)
